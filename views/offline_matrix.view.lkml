view: offline_matrix {
  derived_table: {
    sql: select
      ...
      and PRODUCT_SKU <> ''
      ) as same_day on IL.COMPANYCD = same_day.ERP_COUNTRY_CODE and IL.SKU = same_day.SKU and IL.BRANCHCUSTOMERNBR = same_day.INGRAM_RESELLER_ID and DATE(IL.ENTRYDT) = CAST(same_day.DATES AS DATE FORMAT 'MONTH DD, YYYY')
      left join (
      select distinct Country, Ingram_Reseller_ID, PRODUCT_SKU
      ,case when STRPOS(PRODUCT_SKU,'_') = 3 then (split(PRODUCT_SKU, '_'))[safe_ordinal(2)] else PRODUCT_SKU end as SKU
      ,Dates
      ...
      where Ingram_Reseller_ID <> ''
      and PRODUCT_SKU <> ''
      ) as previous_day on IL.COMPANYCD = previous_day.ERP_COUNTRY_CODE and IL.SKU = previous_day.SKU and IL.BRANCHCUSTOMERNBR = previous_day.INGRAM_RESELLER_ID and DATE(DATE_ADD(IL.ENTRYDT, INTERVAL -1 DAY)) = CAST(previous_day.DATES AS DATE FORMAT 'MONTH DD, YYYY')
      left join (
      select distinct Country, Ingram_Reseller_ID, PRODUCT_SKU
      ,case when STRPOS(PRODUCT_SKU,'_') = 3 then (split(PRODUCT_SKU, '_'))[safe_ordinal(2)] else PRODUCT_SKU end as SKU
      ,Dates
      ...
      where Ingram_Reseller_ID <> ''
      and PRODUCT_SKU <> ''
      ) as twoprevious_days on IL.COMPANYCD = twoprevious_days.ERP_COUNTRY_CODE and IL.SKU = twoprevious_days.SKU and IL.BRANCHCUSTOMERNBR = twoprevious_days.INGRAM_RESELLER_ID and DATE(DATE_ADD(IL.ENTRYDT, INTERVAL -2 DAY)) = CAST(twoprevious_days.DATES AS DATE FORMAT 'MONTH DD, YYYY')
